...
# -------------------------
# Waiting-time feature wrapper (no env file changes)
# -------------------------
class WaitFeatureWrapper:
    """
    Provides:
      - avg waiting time per incoming lane among QUEUED vehicles (speed<thr),
        where waiting time = consecutive queued seconds.
      - state = [phase, time_in_phase, avg_wait_N, avg_wait_S, avg_wait_E, avg_wait_W]
      - reward = -sum(avg_waits) - switch_penalty*(action != prev_action)
    """
    def __init__(self, env: CityFlowSingleIntersectionEnv, switch_penalty: float = 0.1):
        self.env = env
        self.switch_penalty = switch_penalty
        self._wait_consec = {}  # vid -> consecutive queued seconds
        self._prev_action = 0
    ...
    def reset(self, seed=None):
        ...
        self._wait_consec = {}
        self._prev_action = 0
        # Update wait dict based on initial state (optional)
        self._update_waits()
    ...
    def step(self, action: int):
        ...
        avg_waits = state[2:]  # 4 dims
        reward = -float(np.sum(avg_waits))
    ...
    def _update_waits(self):
        ...
        # update consecutive queued time (global per vehicle)
        for lane_id in self.env.IN_LANES:
            for vid in lane_vehicles.get(lane_id, []):
                if vehicle_speeds.get(vid, 0.0) < thr:
                    ...
                    # not currently queued => reset consecutive queue time
    ...
    def _avg_wait_per_lane(self) -> np.ndarray:
        ...
        avgs = []
        for lane_id in self.env.IN_LANES:  # N,S,E,W order
            queued_vids = []
            for vid in lane_vehicles.get(lane_id, []):
                if vehicle_speeds.get(vid, 0.0) < thr:
    ...
    def _get_state(self) -> np.ndarray:
        ...
        return np.concatenate([np.array([phase, phase_t], dtype=np.float32), avg_waits], axis=0)
...
def train(train_flow_file: str, seed: int, switch_penalty: float, cfg: DQNConfig):
    ...
    wenv = WaitFeatureWrapper(env, switch_penalty=switch_penalty)

    state_dim = 2 + 4  # phase, phase_time, 4 avg waits
    ...
    for ep in range(1, cfg.episodes + 1):
        ...
        pbar.close()
        tqdm.write(f"[DQN] ep={ep} return={ep_return:.2f} eps={epsilon_by_step(global_step, cfg):.3f}")
...
def test(test_flow_file: str, seed: int):
    ...
    env = CityFlowSingleIntersectionEnv(base_config_path=base_config_path, flow_path=flow_path, seed=seed)
    wenv = WaitFeatureWrapper(env, switch_penalty=0.0)
    ...
    out = os.path.join("logs", f"DQN_{flow_tag(test_flow_file)}.json")
