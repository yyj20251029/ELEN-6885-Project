...
class WaitFeatureWrapper:
    ...
    State:
      [phase, time_in_phase, Q_NS, Q_EW, W_NS, W_EW]
    ...
    def step(self, action: int):
        ...
        Q_NS, Q_EW, W_NS, W_EW = state[2:]

        reward = -(Q_NS * W_NS + Q_EW * W_EW)
        if action != self._prev_action:
            reward -= self.switch_penalty
        self._prev_action = action

        return state, float(reward), done, info, obs
    ...
    def _get_state(self) -> np.ndarray:
        ...
        for lane_id in self.env.IN_LANES:
            is_ns = lane_id in self.env.IN_LANES[:2]  # N,S
            for vid in lane_vehicles.get(lane_id, []):
                if vehicle_speeds.get(vid, 0.0) < thr:
                    if is_ns:
                        Q_NS += 1
                        W_NS.append(self._wait_consec.get(vid, 0))
                    else:
                        Q_EW += 1
                        W_EW.append(self._wait_consec.get(vid, 0))

        W_NS = float(np.mean(W_NS)) if W_NS else 0.0
        W_EW = float(np.mean(W_EW)) if W_EW else 0.0

        phase = float(self.env.current_phase)
        phase_t = float(self.env.time_in_phase)

        return np.array(
            [phase, phase_t, float(Q_NS), float(Q_EW), W_NS, W_EW],
            dtype=np.float32,
        )
...
def train(train_flow_file: str, seed: int, switch_penalty: float, cfg: DQNConfig):
    ...
    for ep in range(1, cfg.episodes + 1):
        ...
        pbar.close()
        tqdm.write(f"[DQN_base] ep={ep} return={ep_return:.2f} eps={epsilon_by_step(global_step, cfg):.3f}")
...
def test(test_flow_file: str, seed: int):
    ...
    out = os.path.join("logs", f"DQN_base_{flow_tag(test_flow_file)}.json")
...
def main():
    ...
    p_train.add_argument("flow_file", type=str)
    ...
    p_test.add_argument("flow_file", type=str)
